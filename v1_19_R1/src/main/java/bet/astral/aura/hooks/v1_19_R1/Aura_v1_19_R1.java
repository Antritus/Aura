package bet.astral.aura.hooks.v1_19_R1;
...
public class Aura_v1_19_R1 implements AuraInternal {
	private static final Logger log = LoggerFactory.getLogger(Aura_v1_19_R1.class);
	private AuraUserProvider users = null;
	private final Map<GlowColor, PlayerTeam> globalTeams = new HashMap<>();

	public Aura_v1_19_R1() {
	...
	public void setGlowPacket(Player player, Entity entity) {
		...
		byte oldFlags = entityData.get(NettyHandler_v1_19_R1.FLAGS);
		byte newFlags = (byte) (oldFlags | 0x40);

		SynchedEntityData data = createSyncedData(nmsEntity.getEntityData());
		data.define(NettyHandler_v1_19_R1.FLAGS, newFlags);
